...
export const CALCULATOR_CATEGORIES: { key: string; label: string }[] = [
  ...
  { key: "electrolyte", label: "Acid–Base & Electrolytes" },
  { key: "cardio", label: "Cardiovascular" },
  { key: "ventilation", label: "Ventilation & Oxygenation" },
  { key: "fluid", label: "Fluids & Infusion" },
  { key: "drug", label: "Drug Calculations" },
...
export function runCalculator(tool: { slug: string }, values: Record<string, number | string | undefined>): CalcResult {
  ...
  if (!fn) return { lines: [{ label: "Formula not registered", value: "—" }], warnings: [`No formula registered for ${tool.slug}.`] };
  return fn(values);
}