...
export function RacikanForm({ choices }: { choices: Choice[] }) {
  ...
  return <form onSubmit={submit} className="workspace-panel space-y-5 p-5">
    ...
    <button type="submit" disabled={loading} className="focus-ring rounded-lg bg-accent-button px-5 py-2.5 text-sm font-bold disabled:opacity-50">{loading ? "Menghitung..." : "Hitung racikan"}</button>
