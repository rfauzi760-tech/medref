...
export default function MealPlannerPageClient({ items, categories }: { items: FoodItem[]; categories: string[] }) {
  ...
  const plan = useMemo(() => generateMealPlan(items, target), [target]);
  ...
  const withCustom = useMemo(() => {
    const meals = plan.meals.map((m, mi) => {
      const customs = customItems
        .filter((c) => c.meal === mi)
        .map((c) => {
          const food = items.find((f) => f.id === c.foodId)!;
          ...
          <div className="workspace-panel p-5">
            ...
            {customItems.length > 0 && (
              <div className="mt-2 space-y-1">
                {customItems.map((c, i) => {
                  const f = items.find((x) => x.id === c.foodId)!;
                  return (
