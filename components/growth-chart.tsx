...
const TITLES: Record<string, string> = {
  "weight-for-age": "Weight-for-age (0–60 mo)",
  "length-height-for-age": "Length/Height-for-age (0–60 mo)",
  "bmi-for-age": "BMI-for-age (0–60 mo)",
  "head-circumference-for-age": "Head circumference-for-age (0–60 mo)",
};
    ...
    <div className="workspace-panel p-5">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-[10px] text-zinc-400">
          <span className="inline-block h-2 w-2 rounded-full bg-emerald-500" /> patient
        </div>
      </div>
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
            ...
            <XAxis dataKey="x" type="number" domain={["dataMin", "dataMax"]} tick={{ fontSize: 10, fill: "currentColor" }} label={{ value: indicator.includes("weight-for-") && !indicator.includes("age") ? "Length/height (cm)" : "Age (months)", position: "insideBottom", offset: -2, fontSize: 10 }} />
            ...
            <Tooltip
              ...
              labelFormatter={(l) => (indicator.includes("weight-for-") && !indicator.includes("age") ? `${l} cm` : `${l} mo`)}
            />
            {Z_LINES.map((z, i) => (
              <Line
                key={z}
      ...
      <p className="mt-1 text-[10px] leading-relaxed text-zinc-400">
        Curves: z −3 … +3 (WHO Child Growth Standards). Reference lines use the exact LMS parameters from the WHO standards.
      </p>
    </div>
  );
}
