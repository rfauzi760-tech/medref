...
export default function ImmunizationPage() {
  ...
  const assessments = useMemo(() => {
    if (!dobDate) return [];
    return assessImmunization({ schedule: immunizationSchedule, dob: dobDate, received });
  }, [dobDate, received]);
  ...
  const toggleDose = (vaccineId: string, doseNumber: number) => {
    ...
    setReceived((prev) => {
      ...
      const vaccine = immunizationSchedule.vaccines.find((v) => v.id === vaccineId);
  ...
  return (
    <div>
            ...
            <div className="flex overflow-hidden rounded-full border border-zinc-200 dark:border-zinc-700">
              {(
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-medium transition-colors ${filter === key ? "bg-accent text-white" : "text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800 dark:text-zinc-400"}`}
                >
                  {label}
                </button>
              ))}
      ...
      <div className="mt-8">
        <SourceBlock source={immunizationSchedule.source} lastReviewed="2025-06-01" />
      </div>
    </div>
  );
}
